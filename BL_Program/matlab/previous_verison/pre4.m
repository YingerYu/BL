...
fid2 = fopen('OOO_DATA_Output.txt', 'w', 'n', 'Shift_JIS'); 
...
fclose(fid1);
TCP_RX_data = cell2mat(data_matched); % convert the matched data for plot figure

disp('...useful data has been matched')

disp('Finding the unusual data...')
TCP_RX_data_sort = cell2mat(sortrows(data_matched,2));% sort all data in rows and put them into TCP_RX_data_sort cell
...
OOO_DATA_calculated = cell(0, VariMax);
count = 1;
...
figure(4);
OOO_DATA_calculated_plot = cell2mat(OOO_DATA_calculated);
x = OOO_DATA_calculated_plot(:,1);
y = OOO_DATA_calculated_plot(:,2);
plot(x,y,'or');
hold on;
axis square;
axis([0 10 0 3]);
